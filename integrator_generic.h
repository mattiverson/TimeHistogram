...
namespace Integrator {
    ...
    {
        U64 gridIdx;
        while ((gridIdx = sharedGridIdx.fetch_add(1, std::memory_order_relaxed)) < in.nGrid)
        {
            const float* const __restrict dataStart = in.xData;
            ...
            const float targetMass = in.targetMass[gridIdx];
            const float gridX = in.xGrid[gridIdx];
            float gridYMin = in.lowerY[gridIdx];
            float gridYMax = in.upperY[gridIdx];
            float lowerMass = in.lowerMass[gridIdx];
            float upperMass = in.upperMass[gridIdx];

            for (U64 i = 0; i < 20; ++i)
            {
                const float* __restrict pData = dataStart;
                const float* __restrict pYData = yDataStart;
                const float gridY = 0.5f * (gridYMin + gridYMax);
                float integrals = 0.0f;

                do
                {
                    const float dx = gridX - *pData;
                    const float dy = gridY - *pYData;
                    integrals += WendlandEval(dx, xBandwidth, rXBandwidth) * WendlandIntegral(dy, yBandwidth, rYBandwidth);
                ...
                float mass = integrals;
                const bool isOverTarget = targetMass <= mass;

                lowerMass = isOverTarget ? lowerMass : mass;
                gridYMin = isOverTarget ? gridYMin : gridY;
                upperMass = isOverTarget ? mass : upperMass;
                gridYMax = isOverTarget ? gridY : gridYMax;
            }
            out[gridIdx] = gridYMin;
        }
    }
