...
namespace Integrator
{
    ...
    //
    static void FindDataBounds(float& xMin, float& xMax, float& yMin, float& yMax, const float* const __restrict xData,
                               const float* const __restrict yData, const U64 n)
    {
    ...
    static void LineIntegrals(float* const __restrict out, const LineIntegralsInputs& in)
    {
        ...
        auto pDataBlockEnd = reinterpret_cast<const float*>(reinterpret_cast<uintptr_t>(pDataBlock) + DATA_BLOCK_SIZE);
    ...
    // PRECONDITION: nGrid is divisible by 16, nGrid > 0, nData > 0.
    //
    static void IntegrateToMassWorker(float* const __restrict out, const IntegrateToMassInputs& in,
                                      std::atomic<U64>& sharedGridIdx)
    {
        U64 gridIdx;
        while ((gridIdx = sharedGridIdx.fetch_add(8, std::memory_order_relaxed)) < in.nGrid)
        {
            const float* const __restrict dataStart = in.xData;
            ...
            const __m256 targetMass = _mm256_loadu_ps(in.targetMass + gridIdx);
            const __m256 gridX = _mm256_loadu_ps(in.xGrid + gridIdx);
            __m256 gridYMin = _mm256_loadu_ps(in.lowerY + gridIdx);
            __m256 gridYMax = _mm256_loadu_ps(in.upperY + gridIdx);
            __m256 lowerMass = _mm256_loadu_ps(in.lowerMass + gridIdx);
            __m256 upperMass = _mm256_loadu_ps(in.upperMass + gridIdx);

            for (U64 i = 0; i < 20; ++i)
            {
                const float* __restrict pData = dataStart;
                const float* __restrict pYData = yDataStart;
                const __m256 gridY = _mm256_mul_ps(oneHalf, _mm256_add_ps(gridYMin, gridYMax));
                __m256 integrals = _mm256_setzero_ps();

                do
                {
                    __m256 dx = _mm256_sub_ps(gridX, _mm256_set1_ps(*pData));
                    __m256 dy = _mm256_sub_ps(gridY, _mm256_set1_ps(*pYData));
                    integrals = _mm256_fmadd_ps(WendlandEval(dx, xBandwidth, rXBandwidth),
                                                WendlandIntegral(dy, yBandwidth, rYBandwidth), integrals);
                ...
                __m256 mass = integrals;
                __m256 isOverTarget = _mm256_cmp_ps(targetMass, mass, _CMP_LE_OS);

                lowerMass = _mm256_blendv_ps(mass, lowerMass, isOverTarget);
                gridYMin = _mm256_blendv_ps(gridY, gridYMin, isOverTarget);
                upperMass = _mm256_blendv_ps(upperMass, mass, isOverTarget);
                gridYMax = _mm256_blendv_ps(gridYMax, gridY, isOverTarget);
            }
            _mm256_storeu_ps(out + gridIdx, gridYMin);
        }
    }
